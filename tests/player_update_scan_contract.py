...
need('DO NOT read or return OVR, roles, Playstyle or Special Abilities' in scanner,'update prompt derives OVR locally rather than scanning it')
...
need("scope:'age-skills-derived-ovr-auto-name-match-write-verified'" in app,'automatic save records write-verified age/skills/derived-OVR provenance')
need('P.updateAgeSkillsOnly(target.key' in app,'automatic update uses dedicated stats persistence API')
...
need('updateAgeSkillsOnly' in players and 'D.overallFromSkills(nextSkills,roles)' in players and 'Number(verified.ovr)===nextOvr' in players,'player persistence API derives OVR and re-reads/verifies age, OVR and skills')
