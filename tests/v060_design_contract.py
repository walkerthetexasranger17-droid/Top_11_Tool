...
for needle in ['@media (pointer:coarse) and (min-width:1024px) and (max-width:1180px)','@media (orientation:portrait) and (max-width:699px) and (pointer:coarse)','grid-template-columns:1fr!important;overflow-x:visible!important','.v065-team-plan-body{min-width:0!important}']:
    if needle not in css: errs.append(f'missing v0.6.10 cross-device QA marker: {needle}')
if errs:
    print('FAIL v0.6.10 design contract')
    for e in errs: print('-',e)
    sys.exit(1)
print('PASS v0.6.10 design contract: tablet bottom-nav consistency + phone portrait workspace reflow + prior redesigned pages/assets packaged')
