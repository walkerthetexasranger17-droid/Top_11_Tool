...
if 'v0.6.9' not in html: errs.append('visible UI/runtime version is not v0.6.9')
...
# v0.6.9 Scanner + Update Players + compact portrait contract.
for rid in ['scanWindow','scanQueue','scanReview','manualAddPlayerBtn','saveScannedPlayer','updateQueueOverview','updateStatSquad','updateStatQueue','updateStatDone','updateStatAttention']:
    if rid not in ids: errs.append(f'missing v0.6.9 scanner/update control: {rid}')
if not soup.select_one('#page-add-player.v068-scanner-page .v068-scanner-hero'): errs.append('v0.6.9 Scanner hero missing')
if not soup.select_one('#page-add-player .scanner-workspace'): errs.append('v0.6.9 Scanner workspace missing')
...
if "const CACHE='te-v0-6-9-update-p9" not in sw_text or "e.request.mode==='navigate'" not in sw_text or "cache:'no-store'" not in sw_text: errs.append('v0.4.12 service-worker update/navigation freshness guard missing')
