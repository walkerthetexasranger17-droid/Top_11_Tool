...
if nav!=expected_nav: errs.append(f'navigation shell does not match v0.6.18 direct-touch structure: {nav}')
...
if 'v0.6.18' not in html: errs.append('visible UI/runtime version is not v0.6.18')
...
if "const CACHE='te-v0-6-18-squad-mobile-detail" not in sw_text or "e.request.mode==='navigate'" not in sw_text or "cache:'no-store'" not in sw_text: errs.append('v0.4.12 service-worker update/navigation freshness guard missing')
