...
if 'v0.4.14' not in html: errs.append('visible version is not v0.4.14')
...
if "const CACHE='te-v0-4-14" not in sw_text or "e.request.mode==='navigate'" not in sw_text or "cache:'no-store'" not in sw_text: errs.append('v0.4.12 service-worker update/navigation freshness guard missing')
...
# Scanner v8 exact-reference contract.
for needle,label,src in [
    ("const VERSION=8", "Scanner v8 version marker", scanner),
    ...
    ("exactPlaystyleStateMedia", "direct exact same-playstyle state PNG loader", scanner),
    ("buildAbilityReference", "runtime coloured Special Ability board", scanner),
    ("Standard = RIGHT pale, BOTTOM pale, LEFT pale = 000", "Standard 000 scanner rule", scanner),
...
if (ROOT/'assets/playstyles').exists() or (ROOT/'assets/abilities').exists(): errs.append('legacy playstyle/ability asset folders survived v0.4.14')
