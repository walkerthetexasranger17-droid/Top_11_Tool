...
if nav!=expected_nav: errs.append(f'navigation shell does not match v0.6.29 direct-touch structure: {nav}')
...
if 'v0.6.29' not in html: errs.append('visible UI/runtime version is not v0.6.29')
...
# Current Squad contract: sort/view roster remains; redundant summary cards were retired in v0.6.29.
...
for rid in ['squadStatTotal','squadStatAvg','squadStatHigh','squadStatBalance','squadStatNeeds']:
    if rid in ids: errs.append(f'retired v0.6.29 Squad summary control survived: {rid}')
for removed in ['squadSearch','squadPlaystyleFilter','squadAbilityFilter','squadOvrMin','squadOvrMax','squadIssuesOnly','squadResetFilters']:
    if removed in ids: errs.append(f'removed Squad filter control survived v0.6.29: {removed}')
...
if "const CACHE='te-v0-6-29-header-alpha-hotfix" not in sw_text or "e.request.mode==='navigate'" not in sw_text or "cache:'no-store'" not in sw_text: errs.append('v0.4.12 service-worker update/navigation freshness guard missing')
