...
with sync_playwright() as pw:
    ...
    for name,media,hero,grid in [
      ...
      ('squad','.squad-hero-media','.v062-squad-hero','.squad-management-actions')]:
        ...
        if name=='dashboard' and abs(d['grid']['top']-302)>1.1: errs.append(f'{name}: glance Y {d["grid"]["top"]}')
        if name=='squad' and d['grid']['top']>330: errs.append(f'{name}: action row not pulled upward: {d["grid"]["top"]}')
    ...
    if page.locator('#page-dashboard .home-hero-branding').count()!=1: errs.append('Home branded overlay missing')
...
if errs:
    print('FAIL v0.6.27 mobile hero browser contract')
    for e in errs: print('-',e)
    raise SystemExit(1)
print('PASS v0.6.27 browser: compact aligned hero rhythm, full fixed backgrounds, branded Home overlay, functional Squad title, no hero seam')
