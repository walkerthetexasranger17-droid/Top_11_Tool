...
const files=['optimizer-data.js','bible-data.js','data.js','strategy-logic.js','squad-coverage-engine.js','scanner-engine.js','storage.js','drill-profile.js','players.js','training-engine.js','team-training-engine.js','recommendations.js','formation.js','tactics-engine.js','mentor-engine.js','team-plan-engine.js'];
...
(async()=>{
  ...
  const coverageProbe=SCOV.analyse(crafted);ok(coverageProbe.model==='squad-coverage-v1'&&coverageProbe.coveragePercent>0,'squad coverage engine returns deterministic blueprint coverage');ok(coverageProbe.roles.find(x=>x.role==='DMC').deficit>0,'coverage engine identifies missing natural DMC depth in crafted squad');
  const strategicRank=F.rankStrategic(crafted,{opponentSlots:['GK','DC','DC','DC','DMC','MC','MC','AMC','ST','ST','ST']});ok(strategicRank[0].strategic&&Number.isFinite(strategicRank[0].strategic.totalScore),'formation strategic ranking exposes transparent total/components');
  ...
  const autoPlan=TAC.recommendAuto(tacticStarters,{drainLimit:'Medium',opponentSlots:['GK','DC','DC','DC','DMC','MC','MC','AMC','ST','ST','ST'],relativeStrength:-10});ok(autoPlan.autoSelected&&Number.isFinite(autoPlan.decisionScore),'automatic tactics evaluates approach + structure + drain as one decision');ok(autoPlan.contextFit&&Array.isArray(autoPlan.contextFit.reasons),'automatic tactics exposes community/structure context reasons');
  ok(TAC.MODEL_VERSION==='30527-drain-fit-v3-strategy'&&/No claimed private match-engine weights/.test(symmetricPlan.evidencePolicy),'tactics v3 labels heuristic boundary explicitly');
