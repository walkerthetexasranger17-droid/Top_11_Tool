...
if "const CACHE='te-v0-6-8-scanner-p8'" not in sw: errs.append('service-worker cache is not v0.6.7 design cache')
if '?r=0608' not in html: errs.append('v0.6.7 cache-buster missing')
for q in re.findall(r'(?:src|href)="([^\"]+\?r=([^\"]+))"', html):
    if q[1] != '0608': errs.append(f'stale runtime query marker: {q[0]}')
if '?build=0.6.8' not in (ROOT/'START_APP.ps1').read_text(encoding='utf-8'): errs.append('PowerShell launcher build marker is stale')
if 'Top Eleven Tool v0.6.8' not in (ROOT/'START_APP.bat').read_text(encoding='utf-8'): errs.append('Windows launcher title is stale')
if "runtime:'0.6.8'" not in cloud: errs.append('cloud ready diagnostic runtime is stale')
...
for doc in ['START_HERE.md','DESIGN_RECOVERY_HANDOFF_v0.6.8.md','docs/continuity/CURRENT_STATE.md']:
    text=(ROOT/doc).read_text(encoding='utf-8')
    if 'v0.6.8' not in text: errs.append(f'{doc} does not identify v0.6.8 design branch')
...
if errs:
    print('FAIL v0.6.8 design/runtime identity contract')
    for e in errs: print('-',e)
    sys.exit(1)
print('PASS v0.6.8 design/runtime identity: UI/runtime/cache synchronized; v0.5.17 decision contract and v0.5.15 scoring fingerprint preserved')
