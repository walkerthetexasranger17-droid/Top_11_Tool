...
if "const CACHE='te-v0-6-15-home-fidelity-p15'" not in sw: errs.append('service-worker cache is not v0.6.15 Home fidelity cache')
if '?r=0615' not in html: errs.append('v0.6.14 cache-buster missing')
for q in re.findall(r'(?:src|href)="([^\"]+\?r=([^\"]+))"', html):
    if q[1] != '0615': errs.append(f'stale runtime query marker: {q[0]}')
if '?build=0.6.15' not in (ROOT/'START_APP.ps1').read_text(encoding='utf-8'): errs.append('PowerShell launcher build marker is stale')
if 'Top Eleven Tool v0.6.15' not in (ROOT/'START_APP.bat').read_text(encoding='utf-8'): errs.append('Windows launcher title is stale')
if "runtime:'0.6.15'" not in cloud: errs.append('cloud ready diagnostic runtime is stale')
...
for doc in ['START_HERE.md','DESIGN_RECOVERY_HANDOFF_v0.6.15.md','docs/continuity/CURRENT_STATE.md']:
    text=(ROOT/doc).read_text(encoding='utf-8')
    if 'v0.6.15' not in text: errs.append(f'{doc} does not identify v0.6.15 design branch')
...
if errs:
    print('FAIL v0.6.15 design/runtime identity contract')
    for e in errs: print('-',e)
    sys.exit(1)
print('PASS v0.6.15 design/runtime identity: UI/runtime/cache synchronized; v0.5.17 decision contract and v0.5.15 scoring fingerprint preserved')
