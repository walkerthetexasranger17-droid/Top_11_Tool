...
| Squad / Profile / Scanner | Squad supports search plus role, age, OVR, playstyle and availability filters. Profile shows role/white-skill and playstyle state information. Scanner v2 calibrated numeric engine and save-blocking reconciliation retained; current-role plumbing updated only as required. | PASS |
...
- `tests/core-tests.js`: PASS — 241 assertions.
- `tests/scanner_regression.py`: PASS — 12 screenshots / 204 numerical fields.
- `tests/static_checks.py`: PASS.
- `tests/package_integrity.py`: PASS — 69/69 runtime files precached.
