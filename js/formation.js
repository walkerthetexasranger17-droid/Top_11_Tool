(() => {
  const TE=window.TE5=window.TE5||{};const D=TE.Data,B=TE.BibleData,STRAT=TE.Strategy;
  if(!D||!B||!STRAT)throw new Error('data.js, bible-data.js and strategy-logic.js must load before formation.js');
  ...
  const MODEL_VERSION='30527-role-v3-strategy',FORMATION_SOURCE=[...B.FORMATION_TEMPLATES,...COMMUNITY_FORMATIONS],FORMATIONS=FORMATION_SOURCE.map((f,i)=>({...f,templateOrder:i}));
  ...
  function strategicResult(result,{opponentSlots=null,maxMean=null,maxFloor=null}={}){
    if(result?.error)return result;
    const starters=result.chosen||[],avgMean=result.sumMean/starters.length,avgFloor=result.sumFloor/starters.length,structure=STRAT.scoreFormationStructure(starters,opponentSlots);
    const qualityScore=maxMean?55*(avgMean/maxMean):55,naturalScore=10*(result.exact/starters.length),floorScore=maxFloor?10*(avgFloor/maxFloor):10,playstyleScore=5*(result.playstyleFits/starters.length),structureScore=structure.score,totalScore=qualityScore+naturalScore+floorScore+playstyleScore+structureScore;
    return{...result,strategic:{model:MODEL_VERSION,evidence:'TOP ELEVEN TOOL CALCULATION',totalScore,components:{qualityScore,naturalScore,floorScore,playstyleScore,structureScore},avgMean,avgFloor,structure}};
  }
  function rankStrategic(players,{templateId=null,opponentSlots=null}={}){
    if(players.length<11)return[{error:`You need at least 11 players. You currently have ${players.length}.`}];
    const forms=templateId?FORMATIONS.filter(f=>f.id===templateId):FORMATIONS,raw=forms.map(f=>assign(players,f)),valid=raw.filter(x=>!x.error);
    const maxMean=Math.max(...valid.map(x=>x.sumMean/11),0),maxFloor=Math.max(...valid.map(x=>x.sumFloor/11),0);
    return raw.map(x=>strategicResult(x,{opponentSlots,maxMean,maxFloor})).sort((a,b)=>{
      if(a.error&&!b.error)return 1;if(b.error&&!a.error)return-1;if(a.error&&b.error)return (a.formation?.templateOrder||0)-(b.formation?.templateOrder||0);
      const diff=b.strategic.totalScore-a.strategic.totalScore;return Math.abs(diff)>1e-9?diff:compareFormation(a,b);
    });
  }
  ...
  TE.Formation={MODEL_VERSION,FORMATIONS,COMMUNITY_FORMATIONS,options,suitability,roleStats,roleMean,candidate,assign,choose,rankStrategic,chooseStrategic,strategicResult,applyCoordinates,compareFormation};
})();
