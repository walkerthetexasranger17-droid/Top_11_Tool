(() => {
  ...
  const VERSION=12;
  const NATIONALITY_EXTENSION_VERSION=1;
  ...
  const ROI={
    name:{x:791,y:70,w:534,h:101},ovr:{x:816,y:184,w:184,h:86},age:{x:810,y:276,w:184,h:91},roles:{x:1600,y:276,w:238,h:91},nationalityFlag:{x:680,y:385,w:135,h:105},
  ...
  function canonicalNationalityCode(value){const code=String(value||'').toUpperCase().trim().replace(/[^A-Z-]/g,'');if(['ENG','SCO','WAL','NIR'].includes(code))return code;return /^[A-Z]{2}$/.test(code)?code:'';}
  ...
  async function buildUpdateEvidenceBoard(playerMedia,layout='outfield'){
    ...
    const idGap=gap,idH=180,idNameW=Math.floor((W-M*2-idGap*2)*.56),idAgeW=Math.floor((W-M*2-idGap*2)*.16),idNatW=W-M*2-idGap*2-idNameW-idAgeW;drawFieldCard(ctx,player,'PLAYER NAME — MATCH ONLY',ROI.name,M,y,idNameW,idH);drawFieldCard(ctx,player,'AGE',ROI.age,M+idNameW+idGap,y,idAgeW,idH);drawFieldCard(ctx,player,'NATIONALITY FLAG — OPTIONAL METADATA',ROI.nationalityFlag,M+idNameW+idGap+idAgeW+idGap,y,idNatW,idH);y+=idH+gap;
    if(layout==='gk'){
  ...
  function coreResponseSchema(layout='outfield'){const keys=layout==='gk'?GK_CORE_SKILLS:SKILLS,skillProps={};for(const skill of keys)skillProps[skill]={type:'integer',minimum:0,maximum:520};const totals=layout==='gk'?{type:'object',properties:{goalkeeping:{type:'integer'},physical:{type:'integer'}},required:['goalkeeping','physical']}:{type:'object',properties:{defence:{type:'integer'},attack:{type:'integer'},physical:{type:'integer'}},required:['defence','attack','physical']};return{type:'object',properties:{name:{type:'string'},age:{type:'integer'},ovr:{type:'integer'},roles:{type:'array',items:{type:'string'}},nationalityCode:{type:'string',description:'Visible nationality flag only. Use ENG/SCO/WAL/NIR for UK home nations, otherwise ISO alpha-2 (BR, FR, ES, etc). Return empty string when not visible or uncertain.'},totals,skills:{type:'object',properties:skillProps,required:keys},uncertainFields:{type:'array',items:{type:'string'}},notes:{type:'string'}},required:['name','age','ovr','roles','nationalityCode','totals','skills','uncertainFields','notes']}}
  function updateResponseSchema(layout='outfield'){const keys=layout==='gk'?GK_CORE_SKILLS:SKILLS,skillProps={};for(const skill of keys)skillProps[skill]={type:'integer',minimum:0,maximum:520};const totals=layout==='gk'?{type:'object',properties:{goalkeeping:{type:'integer'},physical:{type:'integer'}},required:['goalkeeping','physical']}:{type:'object',properties:{defence:{type:'integer'},attack:{type:'integer'},physical:{type:'integer'}},required:['defence','attack','physical']};return{type:'object',properties:{name:{type:'string',description:'Visible player name. Used only to match an existing saved player; never used to overwrite the saved identity.'},age:{type:'integer',minimum:15,maximum:60},nationalityCode:{type:'string',description:'Optional visible nationality flag only. Use ENG/SCO/WAL/NIR for UK home nations, otherwise ISO alpha-2. Empty string if uncertain.'},totals,skills:{type:'object',properties:skillProps,required:keys},uncertainFields:{type:'array',items:{type:'string'}},notes:{type:'string'}},required:['name','age','nationalityCode','totals','skills','uncertainFields','notes']}}
...
Then read: age, the visible NATIONALITY FLAG code, GOALKEEPING TOTAL, PHYSICAL TOTAL, all 10 goalkeeper skills and all 5 physical skills.
For nationalityCode use ENG/SCO/WAL/NIR for UK home nations; otherwise ISO alpha-2. If the flag is uncertain/not visible return an empty string. NEVER infer nationality from the player name.
...
IMAGE A is a coordinate-locked update board and is the authority.
Read the visible PLAYER NAME only so the app can match this screenshot to the correct saved squad player. The saved name itself must never be changed.
Then read: age, the visible NATIONALITY FLAG code, DEFENCE TOTAL, ATTACK TOTAL, PHYSICAL TOTAL and all 15 outfield skills.
For nationalityCode use ENG/SCO/WAL/NIR for UK home nations; otherwise ISO alpha-2. If the flag is uncertain/not visible return an empty string. NEVER infer nationality from the player name.
DO NOT read or return OVR, roles, Playstyle or Special Abilities. Do not infer identity from role or stats.
Each crop is labelled. Never borrow a neighbouring value and never recalculate a displayed value.
If the name is not completely legible, include name in uncertainFields rather than guessing.
...
Read name, age, OVR, roles, the visible NATIONALITY FLAG, GOALKEEPING TOTAL, PHYSICAL TOTAL, all 10 goalkeeper skills and all 5 physical skills from IMAGE B only.
...
For nationalityCode use ENG/SCO/WAL/NIR for UK home nations; otherwise ISO alpha-2. Return empty string if the flag is uncertain/not visible. Never infer nationality from the name.
...
Read name, age, OVR, roles, the visible NATIONALITY FLAG, the three totals and all 15 outfield skills from IMAGE B only.
For nationalityCode use ENG/SCO/WAL/NIR for UK home nations; otherwise ISO alpha-2. Return empty string if the flag is uncertain/not visible. Never infer nationality from the name.
Each crop is already labelled. Never borrow a neighbouring value. Do not recalculate anything.
When complete call submit_core_scan exactly once.`}
...
Call submit_ability_slot_scan exactly once.`;}
  ...
  function normaliseResult(core,ps,levelResult,specialAbilities,model,uncertain=[],notes='',layoutHint='outfield',abilityTraining=null){
    ...
    const result={version:VERSION,provider:'google-gemini-live',model,name:normaliseText(core.name),age:Number(core.age),ovr:Number(core.ovr),roles,position:roles[0]||(resolvedLayout==='gk'?'GK':null),nationalityCode:canonicalNationalityCode(core.nationalityCode),layout:resolvedLayout,skills,playstyle:pName?{name:pName,level:lvl,levelName:levelResult}:null,specialAbilities:abilities,confidence:{overall:clamp01(conf),text:clamp01(conf),numbers:clamp01(conf),roles:clamp01(conf),playstyle:pName?clamp01(conf):0,specialAbilities:abilities.length?clamp01(conf):1},raw:{totals:rawTotals,model,warnings:allUncertain,providerResponseVersion:13,nationalityExtensionVersion:NATIONALITY_EXTENSION_VERSION,scannerScope:'full-player-native-resolution-v12',specialAbilityTraining:abilityTraining||{active:false}},repairNotes:notes?[notes]:[],validation:{resolved:false,unresolvedChecks:[]}};
  ...
  async function scanUpdate(dataUrl,onProgress=()=>{},options={}){
    ...
    const problems=[];if(!detectedName)problems.push('name missing');if(!Number.isFinite(age)||age<15||age>60)problems.push('age invalid');if(missing.length)problems.push(`${missing.length} skill values missing`);if(outOfRange.length)problems.push(`${outOfRange.length} skill values outside 0-520`);const uncertain=[...new Set(core?.uncertainFields||[])];const result={version:VERSION,provider:'google-gemini-live',model:MODEL,updateOnly:true,name:detectedName,age,nationalityCode:canonicalNationalityCode(core?.nationalityCode),layout,skills,confidence:{overall:uncertain.length?Math.max(.65,.95-uncertain.length*.05):.96,text:(uncertain.some(x=>String(x).toLowerCase().includes('name'))?.75:.97),numbers:uncertain.length?.9:.96},raw:{totals,model:MODEL,warnings:uncertain,providerResponseVersion:15,nationalityExtensionVersion:NATIONALITY_EXTENSION_VERSION,scannerScope:'existing-player-name-match-age-skills-native-resolution-v2',sourceDimensions:{width:playerMedia.sourceWidth||BASE_W,height:playerMedia.sourceHeight||BASE_H},nativeDimensions:{width:playerMedia.sourceWidth||BASE_W,height:playerMedia.sourceHeight||BASE_H},wasNormalised:false,notes:core?.notes||''},checks,validation:{resolved:problems.length===0,unresolvedChecks:problems.map(name=>({name}))}};emit(1,`${detectedName||'Player'} · automatic update scan ready`,'complete');return result;
  }
