...
Call submit_ability_slot_scan exactly once.`;
  ...
  async function scan(dataUrl,onProgress=()=>{},options={}){
    ...
    emit(.03,`Preparing scanner evidence · ${sizeLabel}`);const occupancy=await detectAbilityOccupancy(playerMedia);const[coreBoard,playstyleEvidence]=await Promise.all([buildCoreEvidenceBoard(playerMedia),buildPlaystyleEvidence(playerMedia)]);
