(() => {
  ...
  async function buildUpdateEvidenceBoard(playerMedia,layout='outfield'){
    const player=await requireFrame(playerMedia),W=1800,H=1710,M=24,gap=18,c=document.createElement('canvas');c.width=W;c.height=H;const ctx=c.getContext('2d',{alpha:false});ctx.fillStyle='#fff';ctx.fillRect(0,0,W,H);ctx.fillStyle='#0f172a';ctx.font='bold 31px Arial';ctx.fillText(`EXISTING PLAYER UPDATE — ${layout==='gk'?'GOALKEEPER':'OUTFIELD'} AGE + SKILLS ONLY`,M,42);ctx.font='21px Arial';ctx.fillText('Do not identify the player. Read only AGE, displayed group totals and every labelled skill crop.',M,78);let y=110;
    if(layout==='gk'){
      const topW=Math.floor((W-M*2-gap*2)/3),topH=170;[['AGE',ROI.age],['GOALKEEPING TOTAL',ROI.totals.attack],['PHYSICAL TOTAL',ROI.totals.physical]].forEach((it,i)=>drawFieldCard(ctx,player,it[0],it[1],M+i*(topW+gap),y,topW,topH));y+=topH+gap+8;
      const cols=[[['REFLEXES','reflexes'],['AGILITY','agility'],['ANTICIPATION','anticipation'],['RUSHING OUT','rushingOut'],['COMMUNICATION','communication']],[['THROWING','throwing'],['KICKING','kicking'],['PUNCHING','punching'],['AERIAL REACH','aerialReach'],['CONCENTRATION','concentration']],[['FITNESS','fitness'],['STRENGTH','strength'],['AGGRESSION','aggression'],['SPEED','speed'],['CREATIVITY','creativity']]],cellW=Math.floor((W-M*2-gap*2)/3),cellH=245;cols.forEach((col,ci)=>col.forEach((it,ri)=>drawFieldCard(ctx,player,it[0],GK_SKILL_ROI[it[1]],M+ci*(cellW+gap),y+ri*(cellH+10),cellW,cellH)));
    }else{
      const topW=Math.floor((W-M*2-gap*3)/4),topH=170;[['AGE',ROI.age],['DEFENCE TOTAL',ROI.totals.defence],['ATTACK TOTAL',ROI.totals.attack],['PHYSICAL TOTAL',ROI.totals.physical]].forEach((it,i)=>drawFieldCard(ctx,player,it[0],it[1],M+i*(topW+gap),y,topW,topH));y+=topH+gap+8;
  ...
  function updateResponseSchema(layout='outfield'){const keys=layout==='gk'?GK_CORE_SKILLS:SKILLS,skillProps={};for(const skill of keys)skillProps[skill]={type:'integer',minimum:0,maximum:520};const totals=layout==='gk'?{type:'object',properties:{goalkeeping:{type:'integer'},physical:{type:'integer'}},required:['goalkeeping','physical']}:{type:'object',properties:{defence:{type:'integer'},attack:{type:'integer'},physical:{type:'integer'}},required:['defence','attack','physical']};return{type:'object',properties:{age:{type:'integer',minimum:15,maximum:60},totals,skills:{type:'object',properties:skillProps,required:keys},uncertainFields:{type:'array',items:{type:'string'}},notes:{type:'string'}},required:['age','totals','skills','uncertainFields','notes']}}
...
TASK: update an EXISTING GOALKEEPER from one fresh Top Eleven Skills screenshot.
The player identity is already known by the app. DO NOT read or return name, OVR, roles, Playstyle or Special Abilities.
IMAGE A is a coordinate-locked update board and is the authority.
Read ONLY: age, GOALKEEPING TOTAL, PHYSICAL TOTAL, all 10 goalkeeper skills and all 5 physical skills.
...
TASK: update an EXISTING OUTFIELD PLAYER from one fresh Top Eleven Skills screenshot.
The player identity is already known by the app. DO NOT read or return name, OVR, roles, Playstyle or Special Abilities.
IMAGE A is a coordinate-locked update board and is the authority.
Read ONLY: age, DEFENCE TOTAL, ATTACK TOTAL, PHYSICAL TOTAL and all 15 outfield skills.
Each crop is labelled. Never borrow a neighbouring value and never recalculate a displayed value.
Call submit_player_update exactly once.`}
...
Call submit_ability_slot_scan exactly once.`;}
  ...
  async function scanUpdate(dataUrl,onProgress=()=>{},options={}){
    ...
    const layout=options?.layout==='gk'?'gk':'outfield',raw=dataUrlToMedia(dataUrl),playerMedia=await normaliseFrameMedia(raw),sizeContext=frameSizeContext(playerMedia),emit=(progress,label,phase='update')=>onProgress({progress,label,phase,model:MODEL,layout});
    emit(.06,'Preparing age + skills update');await preflightKeyAndModel(key,MODEL,signal);assertNotAborted(signal);const board=await buildUpdateEvidenceBoard(playerMedia,layout);emit(.24,`${layout==='gk'?'Goalkeeper':'Outfield'} age + skills · HIGH thinking`);
    const core=await runLiveFunctionTask({key,model:MODEL,purpose:'Existing player age and skills update',toolName:'submit_player_update',toolDeclaration:toolDecl('submit_player_update','Submit age, displayed group totals and skill values only for an existing player.',updateResponseSchema(layout)),parts:[{text:sizeContext+'\n'+updatePrompt(layout)+'\nIMAGE A — LABELLED AGE + SKILLS UPDATE BOARD'},inlinePart(board)],signal});
    assertNotAborted(signal);const skills={};for(const[k,v]of Object.entries(core?.skills||{})){const name=canonicalSkillName(k),n=Number(v);if(Number.isFinite(n))skills[name]=n}const age=Number(core?.age),totals=layout==='gk'?{def:null,att:Number(core?.totals?.goalkeeping),phys:Number(core?.totals?.physical)}:{def:Number(core?.totals?.defence),att:Number(core?.totals?.attack),phys:Number(core?.totals?.physical)},required=layout==='gk'?[...D.GK_SKILLS,...D.GK_PHYSICAL]:[...D.OUTFIELD_SKILLS],missing=required.filter(x=>!Number.isFinite(Number(skills[x]))),outOfRange=required.filter(x=>{const v=Number(skills[x]);return Number.isFinite(v)&&(v<0||v>520)}),checks={};
    ...
    const problems=[];if(!Number.isFinite(age)||age<15||age>60)problems.push('age invalid');if(missing.length)problems.push(`${missing.length} skill values missing`);if(outOfRange.length)problems.push(`${outOfRange.length} skill values outside 0-520`);const uncertain=[...new Set(core?.uncertainFields||[])];const result={version:VERSION,provider:'google-gemini-live',model:MODEL,updateOnly:true,age,layout,skills,confidence:{overall:uncertain.length?Math.max(.65,.95-uncertain.length*.05):.96,numbers:uncertain.length?.9:.96},raw:{totals,model:MODEL,warnings:uncertain,providerResponseVersion:13,scannerScope:'existing-player-age-skills-native-resolution-v1',sourceDimensions:{width:playerMedia.sourceWidth||BASE_W,height:playerMedia.sourceHeight||BASE_H},nativeDimensions:{width:playerMedia.sourceWidth||BASE_W,height:playerMedia.sourceHeight||BASE_H},wasNormalised:false,notes:core?.notes||''},checks,validation:{resolved:problems.length===0,unresolvedChecks:problems.map(name=>({name}))}};emit(1,'Age + skills update ready','complete');return result;
  }
  async function healthUpdate(){const key=await getApiKey();if(!key)throw scannerError('NOT_CONFIGURED','Gemini Scanner is not configured. Paste your Google AI Studio API key first.');await preflightKeyAndModel(key,MODEL,null);const ws=await openLiveSocket(key,MODEL,'Update scanner probe',null,null);try{ws.close(1000,'Probe complete')}catch{}return{ok:true,provider:'Gemini 3.1 Flash Live',model:MODEL,thinkingLevel:'HIGH',referenceMode:'age-skills-only-native-resolution'}}
