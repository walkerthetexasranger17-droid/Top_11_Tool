(() => {
  ...
  async function initFirebase(){
    ...
    try{
      ...
      console.info('[Top Eleven Tool] Cloud sync ready',{runtime:'0.6.38',source:state.lastSyncSource,records:state.syncCount,pending:state.pendingWrites,localFirst:localReady});
      return true;
  ...
  function renderAccountPage(){
    ...
    const providers=providerIds(user);
    const linkGoogle=qs('#linkGoogleBtn');if(linkGoogle)linkGoogle.hidden=providers.includes('google.com');
    const resend=qs('#resendVerificationBtn');if(resend)resend.hidden=!!user.emailVerified;
    const currentPasswordField=qs('#accountCurrentPasswordField');if(currentPasswordField)currentPasswordField.hidden=!providers.includes('password');
  }
