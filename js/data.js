(() => {
  ...
  function overallFromSkills(skills,roles){const attrs=applicableSkillsForRoles(roles),vals=attrs.map(a=>skillValue(skills,a));if(!attrs.length||vals.some(v=>v===null))return null;return Math.round(vals.reduce((a,b)=>a+b,0)/vals.length);}
  ...
  const SPECIAL_ABILITY_INTERNAL_ALIASES={'Long Shots':'Shadow Striker','LongShots':'Shadow Striker'};
  const specialAbilityKey=value=>String(value||'').replace(/[^A-Za-z0-9]/g,'').toLowerCase();
  const SPECIAL_ABILITIES=[...new Set(B.SPECIAL_ABILITIES.map(x=>x.name))];
  function canonicalSpecialAbilityName(value){const raw=String(value||'').trim();if(!raw)return'';const key=specialAbilityKey(raw);if(key==='longshots')return SPECIAL_ABILITY_INTERNAL_ALIASES[raw]||'Shadow Striker';if(key==='shadowstriker')return'Shadow Striker';const hit=SPECIAL_ABILITIES.find(x=>specialAbilityKey(x)===key);return hit||raw;}
  const SPECIAL_ABILITY_ROLE_ELIGIBILITY=Object.fromEntries(SPECIAL_ABILITIES.map(name=>[name,[...(B.SPECIAL_ABILITY_ROLE_ELIGIBILITY?.[name]||[])]]));
  function specialAbilitiesForRoles(roles){const natural=new Set((Array.isArray(roles)?roles:[roles]).map(normaliseRole).filter(Boolean));if(!natural.size)return[];return SPECIAL_ABILITIES.filter(name=>(SPECIAL_ABILITY_ROLE_ELIGIBILITY[name]||[]).some(role=>natural.has(role)));}
  function isSpecialAbilityEligibleForRoles(ability,roles){const canonical=canonicalSpecialAbilityName(ability);return !!canonical&&specialAbilitiesForRoles(roles).includes(canonical);}
